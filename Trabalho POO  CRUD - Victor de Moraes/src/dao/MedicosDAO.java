...
public class MedicosDAO {
    ...
    public void alterar (Medicos medicos){
      //Implementação da funcionalidade alterar no DAO
      //(A atualização dos registros)
        ...
        PreparedStatement stmt = null; 
              ...
              try 
        {
            stmt = connection.prepareStatement("update medicos set nome_medicos=?,+"
                    + " Crm_medicos=?, Especialidade_medicos=?, Salario_medicos=? where Id_medicos=? ");
    ...
    // Implementação da funcionalidade deletar no DAO;
   ...
   //Implementação da consulta por nome DAO;
   //( A consulta de um registro por meio do nome e tabela.);

    Medicos consultanome = new Medicos();
        String sql = "select nome_medicos, crm_medicos, especialidade_medicos,"
                + "salario_medicos from medicos where nome_medicos = ?";
        try 
        {
             PreparedStatement stmt = connection.prepareStatement(sql);
            stmt.setString(1,medicos.getNome_medicos());
